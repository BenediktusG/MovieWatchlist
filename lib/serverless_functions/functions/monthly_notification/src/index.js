...
async function fetchMovies() {
  ...
  const url = `https://api.themoviedb.org/3/discover/movie?api_key=${TMDB_API_KEY}&primary_release_date.gte=${startDate}&primary_release_date.lte=${endDate}`;
...
const monthlyMovieNotification = async () => {
  ...
  for (const doc of usersSnapshot.docs) {
    ...
    const body =
      matchedMovies.length > 1
        ...
        : `${matchedMovies[0].title} just released this month — check it out!`;
...
export const handler = async (event) => {
  await monthlyMovieNotification();
};
